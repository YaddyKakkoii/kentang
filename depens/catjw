...
data=$(cat /etc/.trojan | grep '^###' | cut -d ' ' -f 2 | sort | uniq);
LOGO
...
function trojan() {
lane
echo -e "             ${blue}TROJAN USER LOGIN           "
lane
echo -e "  ${yellow}USER          USAGE   TOTAL    LOG"
lane
  for user in ${data[@]}; do
    #echo "${user}"
    logtrojan=$(cat "/var/log/xray/access3.log" | grep -w "email: ${user}" | awk '{print $3}' | sed 's/tcp://g' | tr -d "tcp" | cut -d: -f1 | sed '/^$/d' | cut -d. -f1-3 | sed 's/\./ /g' | tail -n 100 | awk '!seen[$0]++')
    if [[ ${logtrojan} != '' ]]; then
    if [[ ${user} != '' ]]; then
